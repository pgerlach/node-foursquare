...
module.exports = function(config) {
  ...
  function retrieve(url, callback) {
    ...
    method = "GET";
    if (url.substr(config.foursquare.apiUrl.length, 5) == "POST:") {
      method = "POST";
      url = config.foursquare.apiUrl + url.substr(config.foursquare.apiUrl.length + 5);
      console.log("retrieve. this is a POST !. new url is: " + url);
    }
